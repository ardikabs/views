IMAGE_REPOSITORY 	:= ardikabs
IMAGE_VERSION 		:= v1.8.0-rc.1
...
docker-build:
	docker build --no-cache -t $(IMAGE_REPOSITORY)/coredns:$(IMAGE_VERSION) .

docker-push:
	docker push $(IMAGE_REPOSITORY)/coredns:$(IMAGE_VERSION)